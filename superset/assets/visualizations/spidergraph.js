...
require('./spidergraph.css');

// require('./RadarGraph.js');


function SpiderGraph(slice, payload) {
  // $('#code').attr('rows', '15');
  // slice.container.html(payload.data.foo);
  // console.log(payload.data.foo);

  const height = slice.height() - 50;
  // const width = slice.width();
  const width = slice.height() - 50;
  ...
  RadarChart.draw(slice.selector, d, mycfg);
  ...
  //Legend titles
  let LegendOptions = ['Score','Target'];

  let svg = d3.select(slice.selector)
    .selectAll('svg')
    .append('svg')
